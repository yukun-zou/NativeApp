...
class Weather : ViewModel(){
    suspend fun getWeatherAPI(lat: String, long: String): JSONObject {
        return withContext(Dispatchers.IO) {
            var address = "https://api.open-meteo.com/v1/forecast?latitude=€LAT&longitude=€LONG&hourly=apparent_temperature,precipitation&current_weather=true"
    ...
    fun clothingAlgorithm(start: Int, end: Int){
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val temperature = weatherResponse.getJSONObject("hourly").getJSONArray("apparent_temperature")
            val precipitation = weatherResponse.getJSONObject("hourly").getJSONArray("precipitation")
            var minTemp = temperature.getInt(start)
            var maxPri = precipitation.getInt(start)
            for (i in start..end){
                if(temperature.getInt(i) < minTemp){
                    minTemp = temperature.getInt(i)
                }
                if(precipitation.getInt(i) > maxPri){
                    maxPri = precipitation.getInt(i)
                }
            }
            maxPri
            minTemp
